...
const START_GEAR={shoes:0,jacket:0,lamp:0,pack:0,poles:0,hydration:0};
...
function render(){
 ...
 renderLevels();renderShop();renderGear();renderResources();renderLampPower();updateRestUi();drawTrack(0);
}
...
function drawTrack(p){
 ...
 const sky=ctx.createLinearGradient(0,0,0,H);sky.addColorStop(0,'#153554');sky.addColorStop(.65,'#8b5a24');sky.addColorStop(1,'#503a2d');ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);
 // mountains
 ...
 // runner
 const x=50+p*(W-100),y=H*.82;
 ctx.font='64px sans-serif';ctx.fillText('🏃',x-30,y);
 ctx.fillStyle='#fff';ctx.font='22px sans-serif';ctx.fillText(`${(p*L[1]).toFixed(1)} км`,Math.max(10,x-35),y+38);
}
render();
